...
import { ImportResponse, tableConfigImport, tableQLTinTuc_CommentDataTypeTrue } from "@/interface/QLTinTuc_Comment/QLTinTuc_Comment";
...
const Import: React.FC = () => {
    ...
    const [importResult, setImportResult] = useState<tableQLTinTuc_CommentDataTypeTrue[] | null>(null); // State lưu trữ kết quả API

    const handleGetData = useCallback(async () => {
        try {
            const response = await qLTinTuc_CommentService.getDataImportView();
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await qLTinTuc_CommentService.saveImport(dataToSend);
    ...
    const handleDownloadTemplate = async () => {
        const excelBase64 = await qLTinTuc_CommentService.exportTemplateImport();
                ...
                <Col flex={3}>
                    ...
                    <table className={classes.tableCauHinh}>
                        ...
                        {listColumns && listColumns.map((item, index) => {
                            return (
                                <tr key={index}>
                                    <td className={classes.thCss + classes.center}>{index + 1}</td>
        ...
        {isImportSuccessful && importResult && Array.isArray(importResult) && (
            <Card className={classes.customCardShadow}>
                ...
                <table className={classes.tableCauHinh}>
                    ...
                    <tbody>
                        {importResult.map((item: tableQLTinTuc_CommentDataTypeTrue, index: number) => (
                            <tr key={index}>
