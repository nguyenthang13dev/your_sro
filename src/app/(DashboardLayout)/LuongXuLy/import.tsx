...
import { ImportResponse, tableConfigImport, tableLuongXuLyDataTypeTrue } from "@/interface/LuongXuLy/LuongXuLy";
...
const Import: React.FC = () => {
    ...
    const [importResult, setImportResult] = useState<tableLuongXuLyDataTypeTrue[] | null>(null); // State lưu trữ kết quả API

    const handleGetData = useCallback(async () => {
        try {
            const response = await luongXuLyService.getDataImportView();
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await luongXuLyService.saveImport(dataToSend);
    ...
    const handleDownloadTemplate = async () => {
        const excelBase64 = await luongXuLyService.exportTemplateImport();
        ...
        {isImportSuccessful && importResult && Array.isArray(importResult) && (
            <Card className={classes.customCardShadow}>
                ...
                <table className={classes.tableCauHinh}>
                    ...
                    <tbody>
                        {importResult.map((item: tableLuongXuLyDataTypeTrue, index: number) => (
                            <tr key={index}>
