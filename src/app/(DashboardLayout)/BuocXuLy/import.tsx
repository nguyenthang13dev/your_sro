...
import { ImportResponse, tableConfigImport, tableBuocXuLyDataTypeTrue } from "@/interface/BuocXuLy/BuocXuLy";
...
const Import: React.FC = () => {
    ...
    const [importResult, setImportResult] = useState<tableBuocXuLyDataTypeTrue[] | null>(null); // State lưu trữ kết quả API

    const handleGetData = useCallback(async () => {
        try {
            const response = await buocXuLyService.getDataImportView();
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await buocXuLyService.saveImport(dataToSend);
    ...
    const handleDownloadTemplate = async () => {
        const excelBase64 = await buocXuLyService.exportTemplateImport();
        ...
        {isImportSuccessful && importResult && Array.isArray(importResult) && (
            <Card className={classes.customCardShadow}>
                ...
                <table className={classes.tableCauHinh}>
                    ...
                    <tbody>
                        {importResult.map((item: tableBuocXuLyDataTypeTrue, index: number) => (
                            <tr key={index}>
