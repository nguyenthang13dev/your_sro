...
import { ImportResponse, tableConfigImport, tableQLPage_ComponentDataTypeTrue } from "@/interface/QLPage_Component/QLPage_Component";
...
const Import: React.FC = () => {
    ...
    const [importResult, setImportResult] = useState<tableQLPage_ComponentDataTypeTrue[] | null>(null); // State lưu trữ kết quả API

    const handleGetData = useCallback(async () => {
        try {
            const response = await qLPage_ComponentService.getDataImportView();
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await qLPage_ComponentService.saveImport(dataToSend);
    ...
    const handleDownloadTemplate = async () => {
        const excelBase64 = await qLPage_ComponentService.exportTemplateImport();
        ...
        {isImportSuccessful && importResult && Array.isArray(importResult) && (
            <Card className={classes.customCardShadow}>
                ...
                <table className={classes.tableCauHinh}>
                    ...
                    <tbody>
                        {importResult.map((item: tableQLPage_ComponentDataTypeTrue, index: number) => (
                            <tr key={index}>
