...
import {
    ImportResponse,
    tableConfigImport,
    tableTrangThaiLuongXuLyDataTypeTrue,
...
const Import: React.FC = () => {
    ...
    const [isImportSuccessful, setIsImportSuccessful] =
        useState<boolean>(false); // State kiểm tra trạng thái thành công
    const [importResult, setImportResult] = useState<
        tableTrangThaiLuongXuLyDataTypeTrue[] | null
    >(null); // State lưu trữ kết quả API

    const handleGetData = useCallback(async () => {
        try {
            const response =
                await trangThaiLuongXuLyService.getDataImportView();
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await trangThaiLuongXuLyService.saveImport(dataToSend);
    ...
    const handleDownloadTemplate = async () => {
        const excelBase64 =
            await trangThaiLuongXuLyService.exportTemplateImport();
                ...
                <Row>
                    ...
                    <Col flex={3}>
                        <p className={classes.center}>
                            Cấu hình cột lấy thông tin
                        </p>
                        <table className={classes.tableCauHinh}>
                            <tr>
                                <th className={classes.thCss + classes.center}>
                                    #
                                </th>
                            ...
                            <tr>
                                <th
                                    className={classes.thCss + classes.center}
                                ></th>
                                <th className={classes.thCss}>
                                    Dòng bắt đầu đọc
                                </th>
                    ...
                    <Card className={classes.customCardShadow}>
                        ...
                        <table className={classes.tableCauHinh}>
                            ...
                            <tbody>
                                {importResult.map(
                                    (
                                        item: tableTrangThaiLuongXuLyDataTypeTrue,
                                        index: number,
                                    ) => (
                                        <tr key={index}>
                                            {listColumns.map(
                                                (
                                                    column: tableConfigImport,
                                                    colIndex,
                                                ) => {
                                                                ...
                                                                key &&
                                                                    (
                                                                        item as Record<
                                                                            ...
                                                                            any
                                                                        >
                                                                    )[key] // Ép kiểu item để truy cập động
                                                            }
                                                        </td>
                                                    );
                                                },
