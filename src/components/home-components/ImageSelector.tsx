...
const ImageSelector = () => {
  ...
  const handleGetSelectedImg = (
    ...
    const data =
      newsGroupData.find((g) => g.groupName == value)?.items[0] ?? null;
    if (data != null) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      handleGetSelectedImg(
        images[nextIndex],
        labels[nextIndex]?.label,
        labels[nextIndex]?.value
      );
    }, 5000);
  ...
  return (
    <Row gutter={10}>
      <Col
        span={18}
        className="relative bg-center bg-contain bg-no-repeat transform  transition-all duration-300"
        style={{ backgroundImage: `url(${selectedImage})`, height: "500px" }}
      ></Col>
      <Col span={6}>
